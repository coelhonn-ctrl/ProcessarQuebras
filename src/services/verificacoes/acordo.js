...
export class Acordo {
    async haQuantos(page) {
        ...
        try {
            ...
            if (resultado?.umAcordo) {
                console.log(`✅ Acordo único detectado: ${resultado.codigo} (${resultado.total} linhas).`);
    ...
    async buscarBotaoBoletoPorData(frame, dataAlvo) {
        return await frame.evaluate((vencimento) => {
            ...
            const btn = linha.querySelector('a[id*="BOLIMP"], a[id*="IMP"], img[id*="IMP"], a[onclick*="Impressao"]');
            
            if (btn) {
                // Se o botão não tiver ID (raro no Siscobra), criamos um
                if (!btn.id) btn.id = 'click_boleto_' + Math.floor(Math.random() * 1000);
                console.log(`👀 Achei o botão de impressão: ${btn.id} para a data ${alvo}`);
                return btn.id;
            }
    ...
    async extrairDadosGridAdaptado(tabelaHandle) {
        if (!tabelaHandle) throw new Error("Handle da tabela não fornecido para extração.");
    ...
    async verificar_data_de_vencimento(tabela_objeto, vencimento) {
        ...
        function formatarDataVencimento(data) {
            if (!data || typeof data !== 'string') {
                console.error("❌❌❌ ERRO: Data de vencimento inválida! Valor recebido:", data);
                return '';
            }
            const partes = data.split('-');
            if (partes.length !== 3) {
                console.error("❌❌❌ ERRO: Data de vencimento em formato incorreto! Valor recebido:", data);
                return '';
            }
            const [ano, mes, dia] = partes;
            if (!ano || !mes || !dia) {
                console.error("❌❌❌ ERRO: Data de vencimento incompleta! Valor recebido:", data);
                return '';
            }
        ...
        // Exemplo de uso:
        const vencimentoFormatado = formatarDataVencimento(vencimento);

        for (let i = 0; i < tabela_objeto.length; i++) {
            const linha = tabela_objeto[i];
            console.log(`➡️ [Linha ${i}] Vencimento na tabela: "${linha.vencimento}"`);
            if (linha.vencimento === vencimentoFormatado) {
                console.log(`✅ [verificar_data_de_vencimento] Linha encontrada! Index: ${i}`, linha);
                return linha;
            }
        }
        console.log("❌ [verificar_data_de_vencimento] Nenhuma linha encontrada com o vencimento:", vencimentoFormatado);
